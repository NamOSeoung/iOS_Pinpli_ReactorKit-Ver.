...
class RootView: BaseView {
    ...
    lazy var tourBtn: UIButton = {
        ...
        button.addTarget(self, action: #selector(tourClickEvent), for: .touchUpInside)
        return button
    }()
    ...
    lazy var kakaoButton: UIButton = {
        ...
        button.tag = 0
        button.addTarget(self, action: #selector(socialClickEvent), for: .touchUpInside)
        return button
    }()
    ...
    var tourEvent:BehaviorRelay<Bool> = BehaviorRelay.init(value: false)
    var kakaoEvent:BehaviorRelay<Bool> = BehaviorRelay.init(value: false)
    var appleEvent:BehaviorRelay<Bool> = BehaviorRelay.init(value: false)
    ...
    @objc private func tourClickEvent(_ sender: UIButton) {
        sender.showAnimation { [weak self] in
            self?.tourEvent.accept(true)
        }
    }
    @objc private func socialClickEvent(_ sender: UIButton) {
        sender.showAnimation { [weak self] in
            if sender.tag == 0 { //카카오
                self?.kakaoEvent.accept(true)
            }else { //애플
                self?.appleEvent.accept(true)
            }
        }
    }
}
